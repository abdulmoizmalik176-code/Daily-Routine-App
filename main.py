...
DATA_DEFAULT = {
    ...
    "points": 0, "level": 1, "badges": [], "exams": [], "projects": [], "notes": {},
    "points_awarded_log": {}, "dark_mode": False
}
...
def parse_flexible_date(dt_str):
    """Parses various date formats like YYYY-MM-DD, MM-DD, YYYY/MM/DD safely."""
...
def show_popup(title, message, on_close=None):
    ...
    close_btn = Button(text="OK", size_hint_y=None, height=dp(42), background_normal="", background_color=ORANGE, color=WHITE, bold=True)
...
def check_level_up():
    ...
    if new_level > data.get("level", 1):
        ...
        show_popup("Level Up!", f"Congratulations! You are now Level {data['level']}!")
...
def add_fab(root, target):
    fab = FAB(target)
    def reposition(*a): fab.pos = (root.width - fab.width - dp(20), dp(90))
...
def header(title, back_to=None, c1=ORANGE, c2=GOLD):
    box = BoxLayout(orientation="vertical", size_hint_y=None, height=dp(125), padding=[dp(20), dp(10), dp(20), dp(10)])
...
class HomeScreen(SafeScreen):
    ...
    def _build(self):
        ...
        stats.add_widget(make_label(f"Streak: {real_streak} days", fs=dp(16), color=ORANGE, bold=True))
        stats.add_widget(make_label(f"Points: {data.get('points', 0)}", fs=dp(16), color=PURPLE, bold=True))
        stats.add_widget(make_label(f"Level: {data.get('level', 1)}", fs=dp(16), color=GOLD, bold=True))
        ...
        btn_color = (0.8, 0.1, 0.1, 1) if self.timer_running else PINK

        self.timer_lbl = Label(text=timer_text, font_size=dp(32), color=PINK, bold=True, size_hint_x=None, width=dp(120))
        pcard.add_widget(self.timer_lbl)
        self.pom_btn = make_button(btn_text, h=dp(50), bg=btn_color)
        ...
        for i, tk in enumerate(data.get("tasks", [])):
            ...
            col.add_widget(make_label(tk.get("name", "Task"), fs=dp(16), bold=True, h=dp(25)))
            col.add_widget(make_label(f"{tk.get('time', '')} - Progress: {tk.get('progress', 0)}%", fs=dp(12), color=t["muted"], h=dp(20)))

            detail_btn = Button(text="Details", size_hint_x=None, width=dp(60), height=dp(35),
                                background_normal="", background_color=TEAL, color=WHITE, font_size=dp(11), bold=True, pos_hint={"center_y": 0.5})
...
def show_task_details(index, refresh_callback=None, *args):
    ...
    save_btn = Button(text="Save Changes", background_normal="", background_color=ORANGE, color=WHITE, bold=True)
...
class HabitsScreen(SafeScreen):
    def _build(self):
        ...
        for idx, h in enumerate(data.get("habits", [])):
            color = tuple(h.get("color", ORANGE))
            card = Card(orientation="vertical", height=dp(140))
            top = BoxLayout(size_hint_y=None, height=dp(30))
            top.add_widget(make_label(h.get("name", "Habit"), fs=dp(16), bold=True, h=dp(30)))
...
class TasksScreen(SafeScreen):
    def _build(self):
        ...
        for i, tk in enumerate(data.get("tasks", [])):
            ...
            col = BoxLayout(orientation="vertical")
            col.add_widget(make_label(tk.get("name", "Task"), fs=dp(16), bold=True, h=dp(25)))
            col.add_widget(make_label(f"{tk.get('time', '')} | Note: {tk.get('note', '')[:12]}...", fs=dp(12), color=t["muted"], h=dp(20)))
            row.add_widget(col)
            ...
            detail_btn = Button(text="View", size_hint_x=None, width=dp(50), height=dp(35),
                                background_normal="", background_color=TEAL, color=WHITE, font_size=dp(11), bold=True, pos_hint={"center_y":0.5})
            detail_btn.bind(on_release=partial(show_task_details, i, self.on_pre_enter))
            row.add_widget(detail_btn)
...
class AddTaskScreen(SafeScreen):
    ...
    def save_task(self, inst):
        ...
        data.setdefault("tasks", []).append({
            ...
            "recurring": self.recur, "note": self.note_input.text,
            "done_dates": [], "progress": 0, "date_added": str(date.today())
        })
        save_data()
        show_popup("Task Added Successfully", f"Task '{n}' was created and saved!", on_close=lambda: go_screen("tasks"))

class ReportsScreen(SafeScreen):
    mode = "weekly"
    def _build(self):
        self.clear_widgets(); root = BoxLayout(orientation="vertical")
        root.add_widget(header("Reports & History", c1=BLUE, c2=(0.27,0.51,0.70,1)))
        scroll = ScrollView(); content = BoxLayout(orientation="vertical", size_hint_y=None, padding=dp(15), spacing=dp(10)); content.bind(minimum_height=content.setter("height"))
        today = date.today()
        ...
        ent_count = sum(1 for d in data.get("entertainment_log", []) if in_range(d))
        ...
        # SAVED DAILY MOOD NOTES HISTORY (FULL MOOD & NOTE VISIBILITY FIX)
        content.add_widget(section_label("Saved Daily Mood Notes History", color=BLUE))
        if not data.get("notes"):
            ...
            for d, entry in sorted(data.get("notes", {}).items(), reverse=True):
                card = Card(orientation="vertical", height=dp(110), padding=dp(12), spacing=dp(5))
                top_line = BoxLayout(size_hint_y=None, height=dp(25))
                top_line.add_widget(make_label(f"Date: {d}", fs=dp(13), bold=True, h=dp(25)))
                top_line.add_widget(make_label(f"Mood: {entry.get('mood', 'Normal')}", fs=dp(13), color=GOLD, bold=True, h=dp(25)))
                card.add_widget(top_line)

                note_txt = entry.get('note', '').strip() or "(No written note)"
                note_lbl = Label(text=f"Note: {note_txt}", font_size=dp(12), color=WHITE,
...
class MoreScreen(SafeScreen):
    def _build(self):
        ...
        for label, s, c in [("Exam Days", "exams", PINK), ("Projects", "projects", BLUE), ("Daily Note / Check-in", "checkin", GOLD), ("Namaz & Quran", "namaz", ORANGE), ("Historical Work Viewer", "history", TEAL)]:
            btn = make_button(label, h=dp(60), bg=c, fs=dp(16))
            btn.bind(on_release=partial(go_screen, s))
            content.add_widget(btn)
        dm = "Light Mode" if data.get("dark_mode") else "Dark Mode"
        dark_btn = make_button(dm, h=dp(60), bg=(0.4,0.4,0.4,1), fs=dp(16))
...
class HistoryScreen(SafeScreen):
    def _build(self):
        ...
        self.date_input = TextInput(text=str(date.today()), size_hint_y=None, height=dp(45), font_size=dp(16))
    ...
    def load_history(self, *args):
        ...
        card1 = Card(orientation="vertical", height=needed_h, padding=dp(15), spacing=dp(8))
        card1.add_widget(make_label(f"Tasks Completed on {target_date}: ({len(done_tasks)})", bold=True, color=TEAL, h=dp(25)))
        for tk in done_tasks:
            card1.add_widget(make_label(f"• {tk.get('name', '')} ({tk.get('time', '')})", fs=dp(13), color=WHITE, h=dp(22)))
        if not done_tasks: card1.add_widget(make_label("No completed tasks logged.", fs=dp(12), color=GRAY, h=dp(22)))
        ...
        card2 = Card(orientation="vertical", height=namaz_h, padding=dp(15), spacing=dp(8))
        card2.add_widget(make_label(f"Namaz Prayed: ({len(namaz_done)}/5)", bold=True, color=ORANGE, h=dp(25)))
        for p in namaz_done:
            card2.add_widget(make_label(f"• {p}", fs=dp(13), color=WHITE, h=dp(22)))
        if not namaz_done: card2.add_widget(make_label("No prayers logged for this day.", fs=dp(12), color=GRAY, h=dp(22)))
        self.history_container.add_widget(card2)

        # 3. Mood & Note
        mood_entry = data.get("notes", {}).get(target_date)
        card3 = Card(orientation="vertical", height=dp(110), padding=dp(12))
        card3.add_widget(make_label("Daily Mood & Note:", bold=True, color=GOLD, h=dp(25)))
        if mood_entry:
            note_lbl = Label(text=f"Mood: {mood_entry.get('mood')} | Note: {mood_entry.get('note')}",
                             font_size=dp(12), color=WHITE, size_hint_y=None, height=dp(50),
                             halign="left", valign="top")
            note_lbl.bind(size=lambda inst, val: setattr(note_lbl, "text_size", (val[0], None)))
            card3.add_widget(note_lbl)
        else:
            card3.add_widget(make_label("No check-in recorded for this day.", fs=dp(12), color=GRAY, h=dp(25)))
...
class ExamsScreen(SafeScreen):
    def _build(self):
        ...
        content.add_widget(make_label("Date (YYYY-MM-DD or MM-DD):", h=dp(25), color=PINK, bold=True))
        ...
        for exam in data.get("exams", []):
            ...
            card = Card(orientation="vertical", height=dp(80), padding=dp(12))
            card.add_widget(make_label(exam.get("subject", "Exam"), fs=dp(15), bold=True, h=dp(25)))
            card.add_widget(make_label(f"Date: {exam.get('date', '')} ({info})", fs=dp(12), color=PINK, h=dp(20)))
            content.add_widget(card)
        scroll.add_widget(content); root.add_widget(scroll); self.add_widget(root)

    def add_exam(self, inst):
        sub_text, dt_text = self.sub.text.strip(), self.dt.text.strip()
        if not sub_text or not dt_text: show_popup("Error", "Enter subject and date."); return
        data.setdefault("exams", []).append({"subject": sub_text, "date": dt_text}); save_data(); self.on_pre_enter()
...
class ProjectsScreen(SafeScreen):
    def _build(self):
        ...
        for i, p in enumerate(data.get("projects", [])):
            ...
            card.add_widget(make_label(p.get("title", "Project"), fs=dp(15), bold=True, h=dp(25)))
            row = BoxLayout(size_hint_y=None, height=dp(35))
            status_text = "Completed" if p.get("completed") else "In Progress"
            row.add_widget(make_label(status_text, fs=dp(12), color=TEAL if p.get("completed") else PINK, h=dp(35)))
    ...
    def add_project(self, inst):
        tit_text, edt_text = self.tit.text.strip(), self.edt.text.strip()
        if not tit_text or not edt_text: show_popup("Error", "Enter title and end date."); return
        data.setdefault("projects", []).append({"title": tit_text, "start": str(date.today()), "end": edt_text, "completed": False}); save_data(); self.on_pre_enter()
...
class CheckinScreen(SafeScreen):
    ...
    def _build(self):
        ...
        content.add_widget(make_label("Select your mood today:", fs=dp(16), bold=True, h=dp(30)))
        ...
        for m in mood_options:
            ...
            btn = make_button(f"{'[SELECTED] ' if is_selected else ''}{m}", h=dp(45), bg=GOLD if is_selected else GRAY)
        ...
        self.note = TextInput(text=existing_note, size_hint_y=None, height=dp(120), multiline=True)
        ...
        if existing:
            ...
            preview_card = Card(orientation="vertical", height=dp(100), padding=dp(12))
            preview_card.add_widget(make_label(f"Saved Today ({today}): Mood - {existing.get('mood')}", fs=dp(14), bold=True, color=GOLD, h=dp(25)))
            
            p_lbl = Label(text=f"Note: {existing.get('note', '')}", font_size=dp(12), color=WHITE, size_hint_y=None, height=dp(45), halign="left", valign="top")
