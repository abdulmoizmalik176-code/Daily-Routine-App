...
# =====================================================
# COLOR THEME - warm/energetic (orange, coral, gold)
# Kivy colors are (r, g, b, a) with values 0-1, not 0-255
# =====================================================
BG_COLOR = (0.98, 0.95, 0.91, 1)        # warm cream background
PRIMARY_COLOR = (0.91, 0.35, 0.05, 1)   # deep orange (main buttons)
ACCENT_COLOR = (0.95, 0.66, 0.23, 1)    # gold (secondary/back buttons)
TEXT_DARK = (0.25, 0.16, 0.10, 1)       # warm dark brown text
TEXT_WHITE = (1, 1, 1, 1)

Window.clearcolor = BG_COLOR

# ---------------------------------------------------
# STYLE HELPERS - build consistent buttons/labels
# so every screen looks the same without repeating code
# ---------------------------------------------------

def make_button(text, height=50, bg=PRIMARY_COLOR, fg=TEXT_WHITE, font_size=16):
    btn = Button(
        text=text,
        size_hint_y=None,
        height=height,
...
def make_title(text, height=55, font_size=22, color=PRIMARY_COLOR):
    return Label(text=text, size_hint_y=None, height=height,
                 font_size=font_size, color=color, bold=True)

def make_label(text, height=30, font_size=15, color=TEXT_DARK, bold=False):
    return Label(text=text, size_hint_y=None, height=height,
...
def show_popup(title, message):
    popup = Popup(title=title, content=Label(text=message, color=TEXT_DARK),
                   size_hint=(0.8, 0.4),
...
class MainMenuScreen(Screen):
    def __init__(self, **kwargs):
        ...
        layout.add_widget(make_title("My Daily Routine Manager", height=70, font_size=24))
...
def make_back_button(manager):
    btn = make_button("< Back to Menu", height=45, bg=ACCENT_COLOR, fg=TEXT_DARK)
...
class ViewTodayScreen(Screen):
    def on_pre_enter(self):
        ...
        today = str(date.today())
        content.add_widget(make_label("--- Tasks ---", bold=True, color=PRIMARY_COLOR))
        if not data["tasks"]:
            content.add_widget(make_label("No tasks added yet."))
        for t in data["tasks"]:
            status = "Done" if today in t["done_dates"] else "Not Done"
            note = f" | Note: {t['note']}" if t.get("note") else ""
            content.add_widget(make_label(f"{t['name']} - {t['time']} [{status}]{note}"))

        content.add_widget(make_label("--- Namaz ---", bold=True, color=PRIMARY_COLOR))
        times = get_today_namaz_times()
        done_prayers = data["namaz_log"].get(today, [])
        for p in times:
            status = "Done" if p in done_prayers else "Not Done"
            content.add_widget(make_label(f"{p} ({times[p]}) [{status}]"))

        content.add_widget(make_label("--- Quran ---", bold=True, color=PRIMARY_COLOR))
        q_status = "Done" if today in data["quran_log"] else "Not Done"
        content.add_widget(make_label(f"Quran Recitation [{q_status}]"))

        content.add_widget(make_label(f"--- Entertainment ({ENTERTAINMENT_TIME}) ---",
                                       bold=True, color=PRIMARY_COLOR))
        e_status = "Done" if today in data["entertainment_log"] else "Not Done"
        content.add_widget(make_label(f"Status: [{e_status}]"))
...
class AddTaskScreen(Screen):
    def on_pre_enter(self):
        ...
        layout.add_widget(make_label("Task Name:", height=25))
        self.name_input = TextInput(size_hint_y=None, height=40, multiline=False)
        layout.add_widget(self.name_input)

        layout.add_widget(make_label("Time (e.g. 5:00 PM):", height=25))
        ...
        recur_row.add_widget(Label(text="Repeats daily", color=TEXT_DARK))
        layout.add_widget(recur_row)

        layout.add_widget(make_label("Note (optional):", height=25))
        self.note_input = TextInput(size_hint_y=None, height=40, multiline=False)
        layout.add_widget(self.note_input)

        save_btn = make_button("Add Task")
...
class MarkDoneScreen(Screen):
    def on_pre_enter(self):
        ...
        layout = BoxLayout(orientation="vertical", padding=20, spacing=8)
        layout.add_widget(make_back_button(self.manager))
        layout.add_widget(make_title("What do you want to mark done?", height=60, font_size=17))
...
class MarkTaskScreen(Screen):
    def on_pre_enter(self):
        ...
        back_btn = make_button("< Back", height=45, bg=ACCENT_COLOR, fg=TEXT_DARK)
        ...
        for i, t in enumerate(data["tasks"]):
            status = "Done" if today in t["done_dates"] else "Not Done"
            btn = make_button(f"{t['name']} [{status}]")
...
class MarkNamazScreen(Screen):
    def on_pre_enter(self):
        ...
        for p in self.prayers:
            ...
            btn = make_button(f"{p} ({times[p]}) [{status}]")
...
class ExamsScreen(Screen):
    ...
    def refresh_list(self):
        ...
        today = date.today()
        if not data["exams"]:
            self.list_box.add_widget(make_label("No exams added yet."))
        for exam in data["exams"]:
            ...
            self.list_box.add_widget(make_label(txt))
...
class ProjectsScreen(Screen):
    ...
    def refresh_list(self):
        ...
        if not data["projects"]:
            self.list_box.add_widget(make_label("No projects added yet."))
        for i, proj in enumerate(data["projects"]):
            ...
            row.add_widget(Label(text=f"{proj['title']} [{status}]{info}", color=TEXT_DARK))
            if not proj["completed"]:
                done_btn = make_button("Complete", height=40, bg=ACCENT_COLOR, fg=TEXT_DARK, font_size=13)
                done_btn.size_hint_x = 0.4
...
class CheckinScreen(Screen):
    def on_pre_enter(self):
        ...
        layout.add_widget(make_back_button(self.manager))
        layout.add_widget(make_title("How do you feel today?", height=45, font_size=18))

        self.selected_mood = "2"
        mood_grid = GridLayout(cols=1, size_hint_y=None, height=200, spacing=6)
        self.mood_buttons = {}
        for key, val in MOODS.items():
            btn = make_button(val, height=36, bg=ACCENT_COLOR, fg=TEXT_DARK, font_size=14)
        ...
        layout.add_widget(make_label("Write anything about your day:", height=25))
        self.note_input = TextInput(size_hint_y=None, height=120, multiline=True)
        layout.add_widget(self.note_input)

        save_btn = make_button("Save Check-in")
...
class WeeklyReportScreen(Screen):
    def on_pre_enter(self):
        ...
        content.add_widget(make_label(f"Tasks completed this week: {total_tasks_done}"))

        namaz_count = sum(len(data["namaz_log"].get(d, [])) for d in week_dates)
        content.add_widget(make_label(f"Namaz prayed: {namaz_count} / 35"))

        quran_count = sum(1 for d in week_dates if d in data["quran_log"])
        content.add_widget(make_label(f"Quran recitation: {quran_count} / 7 days"))

        ent_count = sum(1 for d in week_dates if d in data["entertainment_log"])
        content.add_widget(make_label(f"Entertainment: {ent_count} / 7 days"))

        content.add_widget(make_label("--- Moods this week ---", bold=True, color=PRIMARY_COLOR))
        for d in week_dates:
            if d in data["notes"]:
                content.add_widget(make_label(f"{d}: {data['notes'][d]['mood']}"))
...
class MonthlyReportScreen(Screen):
    def on_pre_enter(self):
        ...
        content.add_widget(make_label(f"Tasks completed this month: {total_tasks_done}"))

        namaz_count = sum(len(p) for d, p in data["namaz_log"].items() if d.startswith(month_prefix))
        content.add_widget(make_label(f"Namaz prayed: {namaz_count} / {days_passed * 5}"))

        quran_count = sum(1 for d in data["quran_log"] if d.startswith(month_prefix))
        content.add_widget(make_label(f"Quran recitation: {quran_count} / {days_passed} days"))

        ent_count = sum(1 for d in data["entertainment_log"] if d.startswith(month_prefix))
        content.add_widget(make_label(f"Entertainment: {ent_count} / {days_passed} days"))

        completed_projects = sum(1 for p in data["projects"] if p["completed"])
        content.add_widget(make_label(f"Projects completed: {completed_projects} / {len(data['projects'])}"))

        content.add_widget(make_label("--- Moods this month ---", bold=True, color=PRIMARY_COLOR))
        for d, entry in data["notes"].items():
            if d.startswith(month_prefix):
                content.add_widget(make_label(f"{d}: {entry['mood']}"))
...
class PointsScreen(Screen):
    def on_pre_enter(self):
        ...
        layout.add_widget(make_title(f"Points: {data['points']}", height=40, font_size=19))
        layout.add_widget(make_title(f"Level: {data['level']}", height=40, font_size=19, color=ACCENT_COLOR))
        layout.add_widget(make_label("Badges Earned:", bold=True, color=PRIMARY_COLOR, height=30))
        ...
        if not data["badges"]:
            ...
            for b in data["badges"]:
                content.add_widget(make_label(f"- {b}"))
        scroll.add_widget(content)
        layout.add_widget(scroll)
